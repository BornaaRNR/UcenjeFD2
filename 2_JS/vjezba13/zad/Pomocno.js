class Pomocno {
    ...
    static provjeriString(element, minDuljina, porukaGreske) {
    // 1. Provjera postojanja i minimalne duljine
    if (!element || !element.value || element.value.trim().length < minDuljina) {
        throw new Error(porukaGreske);
    }

    // 2. NOVA PROVJERA: Ako vrijednost sadrži razmak bilo gdje u tekstu
    if (element.value.trim().includes(' ')) {
        throw new Error("Unos ne smije sadržavati razmake."); 
        // Možeš proslijediti i novi parametar za poruku ako želiš custom tekst
    }
